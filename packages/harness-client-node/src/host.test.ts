...
test("every shell call starts in the advertised workspace", async () => {
  ...
  try {
    ...
    assert.equal(result.shell?.status, "exited")
    assert.equal(result.shell?.output, realWorkspace)
    assert.equal(host.getWorkspace().root, realWorkspace)
    assert.equal(host.getWorkspace().persistentShell, false)
    assert.equal(host.getWorkspace().shellSessionResults, true)
    const unknown = await host.execute(invocation("shell", { action: "poll", session_id: "sh_missing" }))
    assert.equal(unknown.status, "unknown")
    assert.equal(unknown.shell?.status, "unknown")
  } finally {
    await host.stop()
