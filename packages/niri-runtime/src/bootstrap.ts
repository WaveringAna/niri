...
Every other \`niri\` server method is a coroutine and must be awaited: \`niri.memory\`, \`niri.soul\`, \`niri.context\`, \`niri.discord\`, \`niri.work\`, \`niri.schedule\`, \`niri.aliases\`, \`niri.config\`, and \`niri.webhooks\`. \`await niri.config.get()\` reads your authoritative desired config; \`await niri.config.status()\` reads application status; \`await niri.config.history()\` reads durable revisions. \`await niri.config.update(patch, expected_revision, reason=None, request_id=None)\` saves a durable desired-config update and returns its receipt. The patch is the config fragment itself, named by block or by path — \`{"discord": {"dmWhitelist": "123"}}\` and \`{"discord.dmWhitelist": "123"}\` are the same edit, and there is no outer \`config\` wrapper. \`await niri.webhooks.create(name, expected_revision, signature_header=None, signature_prefix=None, reason=None, request_id=None)\` safely adds a signed webhook with a server-generated secret; its receipt returns the secret, signature format, relative path, and loopback URL. Keep each receipt's request id and supply it when retrying after a deadline; mutations are idempotent. The control plane applies saved revisions automatically: control-plane-only and supported hot changes do not replace the worker, while cold changes wait until your runtime is safe to restart. \`await niri.config.status()\` reports when the active revision has caught up. These surfaces exist only for managed runtimes with a loopback configuration service; otherwise they raise \`NiriUnavailable\`. Compose independent calls with \`asyncio.gather\`. Host failures raise typed exceptions such as \`NiriNotFound\`, \`NiriInvalid\`, \`NiriUnauthorized\`, \`NiriConflict\`, \`NiriDeadlineExceeded\`, and \`NiriUnavailable\`. Use \`help(niri)\`, \`help(niri.memory)\`, or a method's \`__doc__\` when uncertain.
