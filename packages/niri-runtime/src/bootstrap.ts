...
Every other \`niri\` server method is a coroutine and must be awaited: \`niri.memory\`, \`niri.soul\`, \`niri.context\`, \`niri.discord\`, \`niri.work\`, \`niri.schedule\`, \`niri.aliases\`, and \`niri.config\`. \`await niri.config.get()\` reads your authoritative desired config; \`await niri.config.status()\` reads application status; \`await niri.config.history()\` reads durable revisions. \`await niri.config.update(patch, expected_revision, reason=None, request_id=None)\` saves a durable desired-config update and returns its receipt. The patch is the config fragment itself, named by block or by path — \`{"discord": {"dmWhitelist": "123"}}\` and \`{"discord.dmWhitelist": "123"}\` are the same edit, and there is no outer \`config\` wrapper. Keep the receipt's request id and supply it when retrying after a deadline; the update is idempotent. A saved revision is applied by the control plane as soon as your runtime is safe to replace, so a setting takes effect on your next worker start rather than mid-turn; \`await niri.config.status()\` reports when the active revision has caught up. This surface exists only for managed runtimes with a loopback configuration service; otherwise it raises \`NiriUnavailable\`. Compose independent calls with \`asyncio.gather\`. Host failures raise typed exceptions such as \`NiriNotFound\`, \`NiriInvalid\`, \`NiriUnauthorized\`, \`NiriConflict\`, \`NiriDeadlineExceeded\`, and \`NiriUnavailable\`. Use \`help(niri)\`, \`help(niri.memory)\`, or a method's \`__doc__\` when uncertain.
