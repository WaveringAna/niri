...
test("thread creation is deduplicated and messages wait for the mapping", async () => {
  ...
  const forumRequest = __gastownTest.webhookThreadRequest({ id: "hook-1", token: "token-1" }, getDelegatedTask(task.id)!)
  assert.equal(forumRequest.route, "/webhooks/hook-1/token-1")
  assert.equal(forumRequest.body.thread_name, "[researcher] inspect the harness")
  assert.equal(forumRequest.body.username, "researcher")
  assert.match(forumRequest.body.content, /every human member of this private server is equally authorized/)

  updateDelegatedTask(task.id, { status: "interrupted" })
  initDelegation(() => {})
  await new Promise((resolve) => setImmediate(resolve))
  assert.deepEqual(statuses, ["running", "interrupted"])
  assert.match(__gastownTest.webhookThreadRequest({ id: "hook-1", token: "token-1" }, getDelegatedTask(task.id)!).body.thread_name, /^\[interrupted\]/)
})
