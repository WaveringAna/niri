  ...
  for (const clientId of clientIds) {
    for (const clientSecret of clientSecrets) {
      try {
        ...
        // ignore and try next candidate pair
      }
    }
  }
...
// --- Gemini Schema Converter & Model Mapping ---
...
// --- Main Fastify Server Builder ---

export function createBridgeServer(): FastifyInstance {
  const app = Fastify({
    logger: false,
    bodyLimit: 15 * 1024 * 1024 // 15MB max request size
  });
  ...
  // Models route
  const getModels = async (req: any, reply: any) => {
    return reply.send({
      data: [
  ...
  app.post("/v1/chat/completions", async (req: any, reply: any) => {
    ...
    sendTelemetryMetrics(
      ...
      totalLatency
    ); // background async send

    reply.raw.write("data: [DONE]\n\n");
    reply.raw.end();
  });

  // Anthropic messages route (unified POST for both `/v1/messages` and `/messages`)
