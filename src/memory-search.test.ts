...
test("searchMemory includes semantic-only vector candidates and recall accepts them", async (t) => {
  ...
  const moduleUrl = (filePath: string) => pathToFileURL(path.join(process.cwd(), filePath)).href
  const script = `
    import assert from "node:assert/strict"
    import path from "node:path"
    ...
    const [{ initDb, getDb, isVecAvailable, MEMORY_EMBEDDING_DIMENSIONS }, { vectorParam }, search] =
      await Promise.all([
        import(${JSON.stringify(moduleUrl("src/db.ts"))}),
        import(${JSON.stringify(moduleUrl("src/memory/sync.ts"))}),
        import(${JSON.stringify(moduleUrl("src/memory/search.ts"))}),
      ])
